...
doc = Nokogiri::HTML.parse(File.read("detaile.html", :encoding => Encoding::UTF_8), nil, 'UTF-8')
doc.xpath("//table/tr").each do |a|
	...
	a.xpath(".//th").each do |th|
		...
		if key == "社名" || key == "業務内容" || key == "求める経験"
			a.xpath(".//td[#{i}]/div").remove
			val = a.xpath(".//td[#{i}]").inner_text
			print key.tosjis, " : ", val.tosjis, "\n"
			#print val.tosjis, "\n"
		end
		i=i+1
	end
