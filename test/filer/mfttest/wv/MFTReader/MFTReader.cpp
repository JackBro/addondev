...
LARGE_INTEGER num;
   ...
   LoadMFT();
	...
	for(ULONG index=0; index<total_file_count; index++){
		//ULONG index=0;
				...
				switch(attr->AttributeType)
				{
					case AttributeFileName:
						...
						if (fn->NameType & 1 || fn->NameType == 0)
						{
							...
							pfile_info[index].Name = new WCHAR[lstrlenW(fn->Name)+1]; 
							lstrcpyW(pfile_info[index].Name , fn->Name);
						}
						break;
					case AttributeStandardInformation:
						...
						pfile_info[index].CreationTime = si->CreationTime;
						pfile_info[index].LastWriteTime = si->LastWriteTime;
						break;
					case AttributeData:
						if (attr->NameLength==0)
						{
							...
							{
								pfile_info[index].Size = PNONRESIDENT_ATTRIBUTE(attr)->DataSize;
							}
							else
							{
								pfile_info[index].Size = PRESIDENT_ATTRIBUTE(attr)->ValueLength;
							}
						}
						break;
					default:
		...
		if(proc != NULL && (per < index*100/total_file_count)){
			per = index*100/total_file_count;
			abort = proc(per);
			if(abort){
				break;
			}
